...
func NewLogger(service string) {
	serviceName = service
	filePath := fmt.Sprintf(filePathEnv, serviceName, getHostname())
	logLevel, _ = strconv.Atoi(getEnvVar(logLevelEnv, "0"))
	enableLogFile, _ := strconv.ParseBool(getEnvVar(enableLogFileEnv, "true"))
